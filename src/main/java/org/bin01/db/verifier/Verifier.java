...
public class Verifier {
  ...
  public void run(List<QueryPair> queries) throws InterruptedException {
    ...
    while (total < queriesSubmitted) {
      ...
      if (!config.isQuiet() || (progress - lastProgress) > 1) {
        log.info("Progress: %s valid, %s failed, %s skipped, %.2f%% done", valid, failed, skipped,
            progress);
  ...
  private VerifierQueryEvent buildEvent(Validator validator) {
    ...
    if (!validator.valid()) {
      errorMessage =
          format("Test state %s, Control state %s\n", test.getState(), control.getState());
    ...
    return new VerifierQueryEvent(config.getSuite(), config.getRunId(), config.getSource(),
        queryPair.getName(), !validator.valid(), queryPair.getTest().getCatalog(), queryPair
            .getTest().getSchema(), queryPair.getTest().getQuery(), null,
        optionalDurationToSeconds(test), queryPair.getControl().getCatalog(), queryPair
            .getControl().getSchema(), queryPair.getControl().getQuery(), null,
        optionalDurationToSeconds(control), errorMessage);
  }
